...
//		User use = userMapper.getUser();
//		System.out.println("^^^^^^^^^^");
//		System.out.println(use.getName());
//		System.out.println(use.getOrg());
//	}
...
//		user2 = userMapper.getUser(name, passwd);
	...
	public TreeMap<String, TreeMap<String, Integer>> sevenDayNews() {
		...
		// News lastestNews = newsMapper.getLastestOneNews(lang);
		// Date date = lastestNews.getNews_time();
...
//			System.out.println("Date: " + lastestTime);
...
//		SimpleDateFormat sensitiveDate = new SimpleDateFormat("yyyy-MM-dd"); 
	...
	public HashMap<String, String> getEntityPerKey(Integer newsId) {
		...
		criteria.andNews_idEqualTo(newsId);
		// List<EntityTopicRelation> list =
		// entityTopicRelationMapper.selectByExample(example);
		...
		while (it.hasNext()) {
			...
			// String topiciid = etr.getTopic_id();
			Entity entity = entityMapper.selectByPrimaryKey(entityId);
			// Topic topic =topicMapper.selectByPrimaryKey(topiciid);
